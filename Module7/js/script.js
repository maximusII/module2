...
const getTotalBalance = users => {
  const balance = users.reduce((total, user) => user.balance + total, 0);
  return balance;
};
...
const getUniqueSkills = users => {
  const arrAllSkills = users.reduce((acc, user) => {
    acc.push(...user.skills);
    return acc;
  }, []);

  const getUniqueArr = arrAllSkills.filter(function(item, pos) {
    return arrAllSkills.indexOf(item) === pos;
  });

  const sortArray = getUniqueArr.sort();
...
const getNamesSortedByFriendsCount = users => {
  const sortFriendsAmmount = (a, b) => a.friends.length - b.friends.length;
  return users.sort(sortFriendsAmmount).map(user => user.name);
};
